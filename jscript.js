var main = function() {
	$('.search_icon').on('click', function(){
		...
		$('.search_box').animate({width: '50vw'}, 400).css({display: 'block'}).focus();
	});
	$('.search_icon').hover(
		function() {
			$(this).animate({width: '+=10px'}, 400);
		},
		function(){
			$(this).animate({width: '-=10px'}, 400);
		}
	...
	$(document).keyup(function(e){
		...
		if(e.keyCode == 27 ) {
			...
			$('.search_icon').animate({width: '5%'}, 400).fadeIn(0);
		}
			...
			$.ajax({
				...
				success: function(data, status, jqXHR) {
					...
					for(var i = 0; i < data[1].length; i ++) {
						$('.result').append("<div class='container'><div class='well'><a target='blank' href="+data[3][i]+"><h2>"+data[1][i]+"</h2><p>"+data[2][i]+"</p></a></div></div>");
					}
				}
			});
		}
